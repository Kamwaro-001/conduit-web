...
<template>
  ...
  >
    ...
    <div class="p-4 relative">
      <Handle type="target" :position="Position.Left" class="w-3 h-3 bg-slate-500 border-2 border-neutral-800 -ml-1" />

      <!-- Node header -->
      <div class="flex items-start justify-between mb-4">
        <div class="flex items-center gap-3">
          <!-- Icon -->
          <div class="w-8 h-8 rounded bg-slate-800 border border-slate-700 flex items-center justify-center shadow-sm">
            ...
            <svg v-else-if="data.backendType === 'DELAY'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5 text-amber-400">
    ...
    <div v-if="data.backendType === 'CONDITION'" class="flex flex-col gap-2 mt-2">
      <div class="relative text-[10px] text-emerald-400 font-mono px-4 flex justify-between items-center">
        <span>▶ PORT TRUE (ACTIVE)</span>
        <Handle type="source" id="true" :position="Position.Right" class="w-3 h-3 bg-emerald-500 border-2 border-neutral-800 -mr-1.25 top-auto! transform-none! relative" />
      </div>
      <div class="relative text-[10px] text-slate-500 font-mono px-4 flex justify-between items-center">
        <span>▶ PORT FALSE (FALLBACK)</span>
        <Handle type="source" id="false" :position="Position.Right" class="w-3 h-3 bg-slate-500 border-2 border-neutral-800 -mr-1.25 top-auto! transform-none! relative" />
      </div>
    </div>
  </div>
</template>
